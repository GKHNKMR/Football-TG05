...
LEAGUES = {'E0': ('Premier League', 39), 'SP1': ('LaLiga', 140), 'D1': ('Bundesliga', 78), 'I1': ('Serie A', 135), 'F1': ('Ligue 1', 61), 'N1': ('Eredivisie', 88)}
...
def get_csv(code, season):
    ...
    for attempt in range(4):
        try:
            req=Request(url,headers={'User-Agent':'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/120 Safari/537.36','Accept':'text/csv,*/*'})
            with urlopen(req,timeout=30) as r:
                return list(csv.DictReader(io.TextIOWrapper(r,encoding='latin1')))
        except Exception as e:
            print(f'CSV unavailable {code}/{season} attempt {attempt+1}: {e}')
...
def norm(s): return ''.join(c for c in s.lower().strip() if c.isalnum())

def main():
    now=datetime.now(timezone.utc); today=now.date(); end=today+timedelta(days=7); out=[]
    for code,(league,lid) in LEAGUES.items():
        ...
        if not current: print(f'NO CURRENT FIXTURES: {league}'); continue
        for r in current:
            d=parse_date(r.get('Date','')); home=r.get('HomeTeam','').strip(); away=r.get('AwayTeam','').strip()
            if not d or not (today<=d.date()<=end) or not home or not away: continue
            h2h=[x for x in history if {norm(x[1]),norm(x[2])}=={norm(home),norm(away)}]
            h2h.sort(key=lambda x:x[0],reverse=True); h2h=h2h[:10]
            if not h2h: print(f'NO H2H: {home} - {away}'); continue
            ...
            out.append({'match_id':f'{code}-{d:%Y%m%d}-{home}-{away}','league_id':lid,'league':league,'kickoff_utc':d.isoformat().replace('+00:00','Z'),'home':home,'away':away,'h2h_matches':len(h2h),'h2h_total_goal_avg':round(avg,3),'lambda_total':round(avg,3),'p_over_0_5':round(p05,4),'p_over_1_5':round(p15,4),'p_over_2_5':round(p25,4),'label':label,'updated_at':now.isoformat()})
    out.sort(key=lambda x:x['kickoff_utc'])
    if not out: raise RuntimeError('No predictions generated; refusing to publish empty dataset')
    OUTPUT_FILE.write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8')
    print(f'Wrote {len(out)} predictions from 5-year H2H only')
